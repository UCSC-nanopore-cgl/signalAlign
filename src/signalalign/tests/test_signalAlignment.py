...
from py3helpers.utils import create_dot_dict, load_json
...
class SignalAlignmentTest(unittest.TestCase):
    ...
    def test_create_signalAlignment_args(self):
        expected_args = {"backward_reference", "forward_reference", "destination", "stateMachineType", "in_templateHmm",
                         ...
                         'traceBackDiagonals', 'delete_tmp', 'rna', 'ambig_model', "debug", "overwrite"}
    ...
    def test_create_sa_sample_args(self):
        expected_args = {"fofns", "fast5_dirs", "positions_file", "motifs", "bwa_reference", "fw_reference",
                         ...
                         "workers", "assignments_dir", "readdb", "ambig_model"}
    ...
    def test_signal_file_and_alignment(self):
        with captured_output() as (_, _):
            ...
            with tempfile.TemporaryDirectory() as tempdir:
                ...
                args = create_signalAlignment_args(alignment_file=signal_file_guide_alignment,
                ...
                final_args = merge_dicts([args, dict(in_fast5=os.path.join(new_dir,
                                                                           "LomanLabz_PC_20161025_FNFAB42699_MN17633_"
                                                                           "sequencing_run_20161025_E_coli_native_"
                ...
                self.assertEqual(sorted(os.listdir(working_folder.path))[0],
        ...
        with captured_output() as (_, _):
            ...
            with tempfile.TemporaryDirectory() as tempdir:
                ...
                args = create_signalAlignment_args(alignment_file=signal_file_guide_alignment,
                                                   ...
                                                   path_to_bin=self.path_to_bin, destination=working_folder.path,
                                                   embed=True)
                final_args = merge_dicts([args, dict(in_fast5=os.path.join(new_dir, "LomanLabz_PC_20161025_FNFAB42699_"
                                                                                    "MN17633_sequencing_run_20161025_"
                                                                                    "E_coli_native_450bps_82361_ch6_"
                                                                                    "read347_strand.fast5"))])
                handle = SignalAlignment(**final_args)
                handle.run()
                f5fh = Fast5(os.path.join(new_dir, "LomanLabz_PC_20161025_FNFAB42699_MN17633_sequencing_run_20161025_"
                ...
                self.assertEqual(len(os.listdir(working_folder.path)), 1)
                self.assertEqual(sorted(os.listdir(working_folder.path))[0], "9e4d14b1-8167-44ef-9fdb-5c29dd0763fd."
                                                                             "sm.backward.tsv")
            ...
            with tempfile.TemporaryDirectory() as tempdir:
                ...
                args = create_signalAlignment_args(alignment_file=signal_file_guide_alignment,
                                                   ...
                                                   path_to_bin=self.path_to_bin, destination=working_folder.path,
                                                   embed=True)
                final_args = merge_dicts([args, dict(in_fast5=os.path.join(new_dir, "LomanLabz_PC_20161025_FNFAB42699"
                                                                                    "_MN17633_sequencing_run_20161025"
                                                                                    "_E_coli_native_450bps_82361_ch6"
                                                                                    "_read347_strand.fast5"))])
                handle = SignalAlignment(**final_args)
                handle.run()
                f5fh = Fast5(os.path.join(new_dir, "LomanLabz_PC_20161025_FNFAB42699_MN17633_sequencing_run"
                                                   "_20161025_E_coli_native_450bps_82361_ch6_read347_strand.fast5"))
                mea = f5fh.get_signalalign_events(mea=True)
                sam = f5fh.get_signalalign_events(sam=True)
                self.assertEqual(mea[0]["raw_start"], 153)
                self.assertEqual(sam[0], "9")
    ...
    def test_multithread_signal_alignment(self):
        with captured_output() as (_, _):
            with tempfile.TemporaryDirectory() as tempdir:
                ...
                filter_reads_generator = filter_reads_to_string_wrapper(filter_reads(self.bam, self.readdb,
                                                                                     [self.test_dir]))
                output_files = multithread_signal_alignment(signal_align_arguments, [], 2,
                                                            forward_reference=self.ecoli_reference,
                                                            filter_reads_to_string_wrapper_funct=filter_reads_generator,
                                                            debug=False)
    ...
    def test_multithread_signal_alignment_samples(self):
        with captured_output() as (_, _):
            ...
            with tempfile.TemporaryDirectory() as tempdir:
                working_folder = FolderHandler()
                test_fast5 = os.path.join(tempdir, "miten_PC_20160820_FNFAD20259_MN17223_mux_scan_AMS_158_R9_WGA_"
                                                   "Ecoli_08_20_16_83098_ch138_read23_strand.fast5")
                num_files = 1
                ...
                samples.append(SignalAlignSample(working_folder=working_folder, **options))
                self.assertRaises(AssertionError, multithread_signal_alignment_samples, samples,
    ...
    def test_SignalAlignSample(self):
        with captured_output() as (_, _):
            with tempfile.TemporaryDirectory() as tempdir:
                ...
                working_folder = FolderHandler()
                working_folder.open_folder(os.path.join(tempdir, "test_dir"))
                SignalAlignSample(working_folder=working_folder, **test_args)
    ...
    def test_rna_reads(self):
        with captured_output() as (_, _):
            ...
            with tempfile.TemporaryDirectory() as tempdir:
                ...
                in_rna_file = os.path.join(self.test_dir_rna, "DEAMERNANOPORE_20170922_FAH26525_MN16450_sequencing_"
                                                              "run_MA_821_R94_NA12878_mRNA_09_22_17_67136_read_36_"
                                                              "ch_218_strand.fast5")
                ...
                for i, event in enumerate(sa_events):
                    kmer = fh.fetch(reference="rna_fake", start=event["reference_index"],
                ...
                in_rna_file = os.path.join(self.test_dir_rna, "DEAMERNANOPORE_20170922_FAH26525_MN16450_sequencing_"
                                                              "run_MA_821_R94_NA12878_mRNA_09_22_17_67136_read_61_"
                                                              "ch_151_strand.fast5")
                final_args = merge_dicts([args, dict(in_fast5=in_rna_file)])
                handle = SignalAlignment(**final_args)
                handle.run()
    ...
    def test_read_in_signal_align_tsv(self):
        with captured_output() as (_, _):
            example = os.path.join(self.HOME, "tests/test_assignment_files/d6160b0b-a35e-43b5-"
                                              "947f-adaa1abade28.sm.assignments.tsv")
        ...
        with captured_output() as (_, _):
            ...
            with tempfile.TemporaryDirectory() as tempdir:
                ...
                args = create_signalAlignment_args(alignment_file=signal_file_guide_alignment,
                                                   ...
                                                   path_to_bin=self.path_to_bin, destination=working_folder.path,
                                                   embed=True, output_format="both", filter_reads=0,
                                                   twoD_chemistry=True,
                                                   in_complementHmm=complement_model, delete_tmp=True)
                final_args = merge_dicts([args, dict(in_fast5=os.path.join(new_dir, "makeson_PC_20160807_FNFAD20242_"
                                                                                    "MN17284_sequencing_run_MA_470_R9_"
                                                                                    "pUC_g_PCR_BC_08_07_16_93165_ch1_"
                                                                                    "read176_strand.fast5"))])
                handle = SignalAlignment(**final_args)
                handle.run()
                f5fh = Fast5(os.path.join(new_dir, "makeson_PC_20160807_FNFAD20242_MN17284_sequencing_run_MA_470_R9_"
                                                   "pUC_g_PCR_BC_08_07_16_93165_ch1_read176_strand.fast5"))
                mea = f5fh.get_signalalign_events(mea=True)
                sam = f5fh.get_signalalign_events(sam=True)
    ...
    def test_multithread_signal_alignment_samples2(self):
        with captured_output() as (_, _):

            with tempfile.TemporaryDirectory() as tempdir:
                # tempdir = "/Users/andrewbailey/CLionProjects/nanopore-RNN/submodules/signalAlign/1testing"
                working_folder = FolderHandler()
                test_fast5 = os.path.join(tempdir, "miten_PC_20160820_FNFAD20259_MN17223_mux_scan_AMS_158_R9_"
                                                   "WGA_Ecoli_08_20_16_83098_ch138_read23_strand.fast5")
                num_files = 1
                copyfile(self.fast5_paths[0], test_fast5)
                working_folder.open_folder(os.path.join(tempdir, "test_dir"))
                # create signalalign args
    ...
    def test_variant_calling_with_multiple_paths_rna(self):
        with captured_output() as (_, _):
            with tempfile.TemporaryDirectory() as tempdir:
                ...
                args = create_signalAlignment_args(alignment_file=self.rna_bam, bwa_reference=self.rna_reference,
                                                   forward_reference=os.path.join(self.HOME,
                                                                                  "tests/test_sequences/fake_"
                                                                                  "rna_replace/backward.fake_"
                                                                                  "rna.ambig_O.fa"),
                                                   backward_reference=os.path.join(self.HOME,
                                                                                   "tests/test_sequences/fake_"
                                                                                   "rna_replace/forward.fake_"
                                                                                   "rna.ambig_O.fa"),
                                                   in_templateHmm=os.path.join(self.HOME,
                                                                               "models/fake_testModelR9p4_5mer_"
                                                                               "acdgt_RNA.model"),
                                                   path_to_bin=self.path_to_bin, destination=working_folder.path,
                                                   embed=False, output_format="full", filter_reads=0,
                                                   twoD_chemistry=False,
                                                   delete_tmp=True, check_for_temp_file_existance=False,
                                                   ambig_model=os.path.join(self.HOME,
                                                                            "tests/test_position_code/test_ambig.model")
                                                   )

                multithread_signal_alignment(args, list_dir(new_dir, ext="fast5"), worker_count=2,
                                             forward_reference=None,
                                             debug=True, filter_reads_to_string_wrapper_funct=None)
                self.assertEqual(len(os.listdir(working_folder.path)), 2)
